...
{ 
  $temp_id = '';
   $client = getWebClient();
   if(!empty($client->client_preferencs) && !empty($client->client_preferencs->template)){
...
{    
    ...
    $subDomain = explode('.', $domain);
    $data = Redis::get($domain.'_CLIENTPREFERENCS');
    return json_decode($data); 
}

    ...
    {   
        if (!is_null($tree) && count($tree) > 0) {
            ...
            foreach ($tree as $node) {
                ...
                $edit_action = route('godpanel.category.edit',$node['id']);
                $delete_route = route('godpanel.category.delete',$node['id']);
                if(!empty($activeCategory)){
                    if (in_array($node['id'], $activeCategory)) {
                        ...
                        $icon =  $node['icon']['image_path'];
                        if (isset($node['translation_one'])) {
                            $this->htmlData .= '<div class="dd3-content"><div class="dd-img d-flex align-items-center"><img class="rounded-circle mr-1" src="' . $icon . '"><a class="openCategoryModal" data-action="'.$edit_action.'" dataid="' . $node["id"] . '" is_vendor="0" href="#"> ' . $node['translation_one']["name"] . '</a></div><span class="inner-div text-right">';
                        } else {
                        ...
                        if (!in_array($node["id"], $blockedCategory)) {
                            ...
                            if ($from == 'category') {
                                if ($node["is_core"] == 1) {
                                    $this->htmlData .= '<a class="action-icon openCategoryModal" data-action="'.$edit_action.'" dataid="' . $node["id"] . '" is_vendor="0" href="#"> <i class="mdi mdi-square-edit-outline"></i></a><a class="action-icon" dataid="' . $node["id"] . '" title="' . $title . '" onclick="' . $askMessage . '" href="' . $delete_route . '"> <i class="mdi ' . $icon . '"></i></a>';
                                }
                            } elseif ($from == 'vendor' && $node["is_core"] == 0) {
                                $this->htmlData .= '<a class="action-icon openCategoryModal" data-action="'.$edit_action.'" dataid="' . $node["id"] . '" is_vendor="1" href="#"> <i class="mdi mdi-square-edit-outline"></i></a>
                                <a class="action-icon" dataid="' . $node["id"] . '" onclick="' . $askMessage . '" href="' . $delete_route . '" title="' . $title . '"> <i class="mdi ' . $icon . '"></i></a>';
                            }
                        }

...
class CartController extends FrontController
{
    ...
    public function showCart(Request $request, $domain = '')
    {
        ...
        $guest_user = true;
        if ($user) {
            $cart = Cart::select('id', 'is_gift', 'item_count','comment_for_pickup_driver','comment_for_dropoff_driver','comment_for_vendor','specific_instructions')->with('coupon.promo')->where('status', '0')
 ...
 $page = getTemplateLayoutPath('modules').'cartnew';


        return view($page,compact('public_key_yoco','cart','client_detail'))->with($data,$client_preference_detail,$client_detail);
    ...
    public function getCartProducts(Request $request,$domain = '')
    {
        ...
        $langId = Session::get('customerLanguage');
        if ($user) {
            $cart = Cart::select('id', 'is_gift', 'item_count', 'schedule_type', 'scheduled_date_time')->with('coupon.promo')->where('status', '0')
            ->where('client_id',$this->client_id)
            ->where('user_id', $user->id)->first();
        } else {
            $cart = Cart::select('id', 'is_gift', 'item_count', 'schedule_type', 'scheduled_date_time')->with('coupon.promo')->where('status', '0')
            ->where('client_id',$this->client_id)
            ->where('unique_identifier', session()->get('_token'))->first();
        }
        if ($cart) {
    ...
    public function getCart($cart, $address_id=0 , $code = 'D')
    {
      ...
      $cart_id = $cart->id;
        ...
        $delifproductnotexist = CartProduct::where('cart_id', $cart_id)->where('client_id',$this->client_id)->doesntHave('product')->delete();
    ...
    {   $cart_details = [];
        ...
        $client_preference_detail = ClientPreference::where('client_id',$this->client_id)->first();

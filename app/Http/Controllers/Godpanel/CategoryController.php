...
use App\Models\{Client, Language, ClientPreference, MapProvider, Category, Category_translation, ClientLanguage, Variant, Brand, CategoryHistory, Type, CategoryTag, Vendor, DispatcherWarningPage, DispatcherTemplateTypeOption, Product,CategoryTranslation};
...
class CategoryController extends BaseController
{
    ...
    private $view_path = 'godpanel.modules.catalog.';
    public $client_id =0;
    public function __construct() { 
        $this->folderName = 'pictures/category/';
    }
    /**
...
#------------------------------------------------------------------------------------------------------------
# Get Add Category Form for Model
#------------------------------------------------------------------------------------------------------------
    ...
    public function create(Request $request)
    {
        ...
        //     ->where('client_languages.is_active', 1)
        //     ->orderBy('client_languages.is_primary', 'desc')->get();
        $langs = Language::select('id as langId','name as langName','sort_code')
           ->where('basic_lang',1)
            ->get();
...
public function store(Request $request)
    ...
    public function save(Request $request, Category $cate, $update = 'false')
    {
        try {
            ...
            $cate->category_type = $request->category_type;
            ...
            if ($request->hasFile('icon')) {
                 ...
                 $cate->icon = $img_path ;//Storage::disk('public')->put($this->folderName, $file, 'public');
            }
            if ($request->hasFile('image')) {
                $file = $request->file('image');
                 $img_path = uploadFileWithAjax23($this->folderName,$file);
                $cate->image =$img_path;// Storage::disk('public')->put('/category/image', $file, 'public');
            }
            $cate->save();
 ...
 /**
    ...
    public function updateOrder(Request $request)
    {
        ...
        if ($arr > 0) {
            return redirect()->route('godpanel.category.index')->with('success', 'Category order updated successfully!');
        }
    }

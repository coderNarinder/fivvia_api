...
class ProcessClientDatabase implements ShouldQueue
{
    ...
    {
        $client = Client::where('id', $this->client_id)->first(['name', 'email', 'password', 'phone_number', 'database_path', 'database_name', 'database_username', 'database_password', 'logo', 'company_name', 'company_address', 'custom_domain', 'status', 'code', 'country_id', 'sub_domain'])
        ->toarray();
        $clientData = array();
        ...
        try {
            ...
            $default = [
                ...
                'database' => $schemaName,
                'username' => $first_client->database_username,//env('DB_USERNAME'),
                'password' => $first_client->database_password,//env('DB_PASSWORD'),
                'charset' => 'utf8mb4',

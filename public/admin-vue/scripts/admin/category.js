 ...
 jQuery("form[name=editCategory]").validate({
    rules: {
      ...
      'label': 
      {
        ...
        'duration_label': 
      {
        required: parseInt($("body").find('#cates').val()) > 0 ? true : false,
      },'price_label': 
      {
        required: parseInt($("body").find('#cates').val()) > 0 ? true : false,
      }, 
 ...
 jQuery("form[name=addCategory]").validate({
    ...
    var dataTable = $('#example2').DataTable({
        ...
        columns: [
             ...
             { data: 'featured', name: 'featured',searchable:false,"bSortable":true},
             ...
             { data: 'menuActive', name: 'menuActive',searchable:false,"bSortable": false, render: function ( data, type, row ){
                    if(parseInt(data.id) > 0){
                      $text  ='<div class="checkbox switcher">';
                      $text +='<label for="test22'+data.id+'">';
                      $text +='<input type="checkbox" class="changeStatusToggle" id="test22'+data.id+'" value="" '+data.checked+' data-action="'+data.url+'">';
                      $text +='<span><small></small></span>';
                      $text +='<small>'+data.label+'</small>';
                      $text +='</label>';
                      $text +='</div> ';
